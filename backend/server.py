import sqlite3
import time
import hashlib
import epub2txt
import os
from datetime import date, datetime, timedelta
from flask_cors import CORS

connection = sqlite3.connect('database.sqlite',check_same_thread=False,isolation_level="DEFERRED")

import os

data_file_path = os.path.join(os.path.dirname(__file__), "schema.sql")
with open(data_file_path) as f:
    connection.executescript(f.read())

from flask import Flask, render_template, request, jsonify
app = Flask(__name__)
base_ebook_path = os.path.join(app.root_path,'ebooks')
CORS(app, origins='*')
def check_valid_token(token, check_admin_token= False):
   with connection:
      cursor=connection.cursor()
      if check_admin_token :
         cursor.execute(f"select * from LOGIN_ADMIN_RECORD where token = '{token}'")
      else:
         cursor.execute(f"select * from LOGIN_RECORD where token = '{token}' and is_active = 1 ") 
      result=cursor.fetchone()
      if result==None:
         return None
      else:
         current_time= datetime.fromtimestamp(time.time())
         max_time= datetime.fromtimestamp(result[2]) + timedelta(minutes=60)
         if current_time>max_time:
            cursor.execute(f"update LOGIN_RECORD set is_active = 0 where token = '{token}'")
            return None
         
         else:
            return result[1] 
      
@app.route('/add',methods=['POST'])
def add_book():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token, check_admin_token= True)
      if user_id==None:
         return 'invalid login'
      book_name=(request.form['book_name'])
      author=(request.form['author'])
      summary=(request.form['summary'])
      genre=(request.form['genre'])
      book_url=(request.form['book_url'])
      cursor.execute(
       "INSERT INTO BOOKS (book_name, author, summary, genre, book_url) VALUES (?, ?, ?, ?, ?)",
       (book_name, author, summary, genre, book_url)
      )
      connection.commit()    
      return "success"

@app.route('/delete',methods=['POST'])
def remove_book():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token, check_admin_token= True)
      if not user_id:
         return 'invalid login'
      book_id=(request.form['book_id'])
      cursor.execute(f"UPDATE BOOKS SET is_deleted = 1 WHERE book_id = '{book_id}' ")
      connection.commit()    
      return "success"

@app.route('/update',methods=['POST'])
def update():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token,True)
      if not user_id:
         return 'invalid login'
      book_id=(request.form['book_id'])
      book_name=(request.form['book_name'])
      author=(request.form['author'])
      summary=(request.form['summary'])
      genre=(request.form['genre'])
      book_url=(request.form['book_url'])
      cursor.execute("UPDATE BOOKS SET book_name=?, author=?, summary=?, genre=?, book_url=? WHERE book_id=?",
       (book_name, author, summary, genre, book_url, book_id)
      )
      connection.commit()    
      return "success"

@app.route('/display')
def show_book():
   with connection:
      cursor=connection.cursor()
      book_id=(request.args.get('book_id'))
      cursor.execute(f"select * from BOOKS where book_id ='{book_id}' AND is_deleted = 0;")
      result=cursor.fetchall()
      result_dictionary={'book_id':result[0][0],'book_name':result[0][1],'author':result[0][2],'summary':result[0][3],'genre':result[0][4],'book_url':result[0][5]} 
      return result_dictionary

@app.route('/search')
def search_book():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token)
      search_term=(request.args.get('search_term'))
      print(search_term)
      cursor.execute(f'select * from BOOKS where book_name like "%'+search_term+'%" AND is_deleted = 0;')
      result=cursor.fetchall()
   
      if result!=None:
         result_dictionary=[]
         for row in result:
            row_dictionary={'book_id':row[0],'book_name':row[1],'author':row[2],'summary':row[3],'genre':row[4]}
            result_dictionary.append(row_dictionary)
      else:
         return []
      if user_id:
            for row in result_dictionary:
               cursor.execute(f"select * from READING where user_id ='{user_id}' and book_id ='{row['book_id']}';")
               result_status=cursor.fetchone()
               if result_status==None: 
                  reading_status = 'not complete'
               else:
                  if result_status[4]==None:
                     reading_status = 'not complete'
                  else:
                     reading_status = 'complete'
            row.update({'reading_status':reading_status})   
      return jsonify(result_dictionary)

@app.route('/display_library')
def show_database():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token)
      genre=(request.args.get('genre'))
      if(genre==None):
         cursor.execute("select * from BOOKS where is_deleted = 0;")
      else:
         cursor.execute(f"select * from BOOKS where genre='{genre}' AND is_deleted = 0;")
      result=cursor.fetchall()
      result_dictionary=[]
      for row in result:
        row_dictionary={'book_id':row[0],'book_name':row[1],'author':row[2],'summary':row[3],'genre':row[4]}
        result_dictionary.append(row_dictionary)
        if user_id:
            for row in result_dictionary:
               cursor.execute(f"select * from READING where user_id ='{user_id}' and book_id ='{row['book_id']}' AND is_deleted = 0;")
               result_status=cursor.fetchone()
               if result_status==None: 
                  reading_status = 'not complete'
               else:
                  if result_status[4]==None:
                     reading_status = 'not complete'
                  else:
                     reading_status = 'complete'
            row.update({'reading_status':reading_status})      
      return jsonify(result_dictionary)

@app.route('/display_genre')
def show_genre():
   with connection:
      cursor=connection.cursor()
      cursor.execute(f"select DISTINCT genre from BOOKS WHERE is_deleted = 0;")
      result=cursor.fetchall()

      result_dictionary=[]
      for row in result:
         if(len(row)>0):
            row_dictionary=row[0]
            result_dictionary.append(row_dictionary)
   
      return jsonify(result_dictionary)


@app.route('/display_trending')
def show_trending():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token)
      cursor.execute(
         """SELECT *
            FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY genre ORDER BY book_id) AS rn
            FROM BOOKS WHERE is_deleted = 0)
            WHERE rn = 1 ;""")
      result=cursor.fetchall()
      result_dictionary=[]
      for row in result:
         row_dictionary={'book_id':row[0],'book_name':row[1],'author':row[2],'summary':row[3],'genre':row[4]}
         result_dictionary.append(row_dictionary)
      if user_id:
         for row in result_dictionary:
            cursor.execute(f"select * from READING where user_id ='{user_id}' and book_id ='{row['book_id']}';")
            result_status=cursor.fetchone()
            if result_status==None: 
               reading_status = 'not complete'
            else:
               if result_status[4]==None:
                  reading_status = 'not complete'
               else:
                  reading_status = 'complete'
            row.update({'reading_status':reading_status})      

      return jsonify(result_dictionary)

@app.route('/register',methods=['POST'])
def add_user():
   with connection:
      cursor=connection.cursor()
      name=(request.form['name'])
      email=(request.form['email'])
      password=(request.form['password'])
      contact_number=(request.form['contact_number'])
      cursor.execute(f"select * from USERS where email ='{email}';")
      result=cursor.fetchone()
      print(result)
      if result!=None:
         return 'email already exists'

      cursor.execute(f"insert into USERS(name, email, password, contact_number) values ('{name}', '{email}', '{password}', '{contact_number}');")
      connection.commit()    
      return "Registration complete"

@app.route('/login',methods=['POST'])
def login_user():
   with connection:
      cursor=connection.cursor()
      email=(request.form['email'])
      password=(request.form['password'])
      cursor.execute(f"select * from USERS where email ='{email}' and password ='{password}';")
      result=cursor.fetchone()
      if result==None:
         return 'wrong email/password'
      else:
         unique_string= str(result[0])+email+str(time.time())
         new_access_token= hashlib.md5(unique_string.encode()).hexdigest()
         cursor.execute(f"insert into LOGIN_RECORD(token,user_id,login_time,is_active) values('{new_access_token}','{result[0]}','{time.time()}',1)")
         result_dictionary={'user_name':result[1],'email':result[2],'contact_number':result[3],'access_token':new_access_token}
         return result_dictionary
      
@app.route('/logout',methods=['POST'])
def logout_user():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token)
      cursor.execute(f"update LOGIN_RECORD set is_active = 0 where user_id = '{user_id}'")
      return 'successful logout'
   
@app.route('/admin_login',methods=['POST'])
def login_admin():
   with connection:
      cursor=connection.cursor()
      admin_id=(request.form['admin_id'])
      password=(request.form['password'])
      cursor.execute(f"select * from ADMIN where admin_id ='{admin_id}' and password ='{password}';")
      result=cursor.fetchone()
      if result==None:
       print('wrong admin id/password')
       return 'wrong admin id/password'
      else:
         unique_string= str(result[1])+admin_id+str(time.time())
         new_access_token= hashlib.md5(unique_string.encode()).hexdigest()
         cursor.execute(f"insert into LOGIN_ADMIN_RECORD(token, admin_id, login_time, is_active) values('{new_access_token}','{result[0]}','{time.time()}',1)")
         result_dictionary={'admin_id':result[0], 'access_token':new_access_token}
         print(result_dictionary)
         return result_dictionary

@app.route('/admin_logout',methods=['POST'])
def logout_admin():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token, check_admin_token= True)
   
      cursor.execute(f"update LOGIN_ADMIN_RECORD set is_active = 0 where admin_id = '{user_id}'")
      return 'successful logout'
   
def get_book_content(book_id, chapter_no):
    with connection:
      cursor=connection.cursor()
      cursor.execute(f"select book_url from BOOKS where book_id ='{book_id}';")
      result=cursor.fetchone()
      print(result)
      if result == None:
         return 'invalid book_id'
      path_to_ebook=os.path.join(base_ebook_path, result[0])
      book_content=epub2txt.epub2txt(path_to_ebook, outputlist=True)
      chap_content=book_content[int(chapter_no)]
      max_chapter=len(book_content)-1
      return chap_content, max_chapter

@app.route('/read/<int:book_id>',defaults={'chapter_no': None})   
@app.route('/read/<int:book_id>/<int:chapter_no>')
def read_book(book_id, chapter_no):
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      today = date.today()
      user_id=check_valid_token(token)
      if not user_id:
         return 'invalid login'
      cursor.execute(f"select * from READING where user_id ='{user_id}' and book_id ='{book_id}';")
      result=cursor.fetchone()
      cursor.execute(f"select book_name from BOOKS where book_id ='{book_id}';")
      book_name=cursor.fetchone()
      if result==None and chapter_no==None:
         cursor.execute(f"insert into READING(book_id, user_id, date_of_start, chapter_no) values ('{book_id}', '{user_id}', '{today}', 0);")
         connection.commit()
         content, max_chapters= get_book_content(book_id, 0)
         result_dictionary={'book_id':book_id, 'book_name':book_name[0], 'date_of_start':today, 'chapter_no': 0, 'content': content, 'max_chapters': max_chapters }
         return result_dictionary
      else:
         content, max_chapters= get_book_content(book_id, chapter_no if chapter_no!= None else result[5])
         result_dictionary={'book_id':book_id, 'book_name':book_name[0], 'date_of_start':result[3],
                           'chapter_no':chapter_no if chapter_no!=None else result[5], 
                           'content': content, 'max_chapters': max_chapters}
         return result_dictionary
      

@app.route('/update_read', methods=['POST'])
def update_book():
   with connection:
      cursor=connection.cursor()
      book_id=(request.form['book_id'])
      reading_status=(request.form['reading_status'])
      if reading_status != 'complete':
         chapter_no=(request.form['chapter_no'])
      token=(request.headers['Authorization'])
      today = date.today()
      user_id=check_valid_token(token)
      print('user_id:',user_id)
      if not user_id:
         return 'invalid login'
      if reading_status=='complete': 
         cursor.execute(f"UPDATE READING SET date_of_end ='{today}' WHERE user_id ='{user_id}' AND book_id ='{book_id}';")
      else:
         cursor.execute(f"UPDATE READING SET chapter_no='{chapter_no}' WHERE user_id ='{user_id}' AND book_id ='{book_id}';")
      connection.commit()  
      return 'Book Updated'

@app.route('/check_reading_status')
def check_reading_status():
   with connection:
      cursor=connection.cursor()
      book_id=(request.args.get('book_id'))
      token=(request.headers['Authorization'])
      user_id=check_valid_token(token)

      if not user_id:
         return 'invalid login'
      cursor.execute(f"select * from READING where user_id ='{user_id}' and book_id ='{book_id}';")
      result=cursor.fetchone()
      if result==None: 
         return 'no record found'
      else:
         if result[4]==None:
            reading_status = 'not complete'
         else:
            reading_status = 'complete'
         
         return reading_status
        

@app.route('/record')
def lending_record():
   with connection.cursor() as cursor:
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token, check_admin_token= True)
      if user_id==None:
         return 'invalid login'
      user_id=(request.args.get('user_id'))
      book_id=(request.args.get('book_id'))
      cursor.execute(f"select * from READING where user_id ='{user_id}' or book_id ='{book_id}';")
      result=cursor.fetchall()
      if result!=[]:   
         result_dictionary=[]
      for row in result:
         row_dictionary={'record_id':row[0],'book_id':row[1],'user_id':row[2],'date_of_start':row[3],'date_of_end':row[4]}
         result_dictionary.append(row_dictionary)
         return jsonify(result_dictionary)
      else:
         return 'wrong user_id/book_id'
      
@app.route('/display_lending')
def show_records():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token, check_admin_token= True)
      if user_id==None:
         return 'invalid login'
      cursor.execute(f"""SELECT
                           READING.record_id,
                           BOOKS.book_name,
                           BOOKS.author,
                           USERS.email,
                           READING.date_of_start,
                           READING.date_of_end
                         FROM READING
                         JOIN BOOKS    ON READING.book_id = BOOKS.book_id
                         JOIN USERS    ON READING.user_id = USERS.user_id;""")
      result=cursor.fetchall()
      result_dictionary=[]
      for row in result:
        row_dictionary={'record_id':row[0],'book_name':row[1],'author':row[2],'user_email':row[3],'date_of_start':row[4], 'date_of_end':row[5]}
        result_dictionary.append(row_dictionary)

      return jsonify(result_dictionary)
   
@app.route('/admin_search_record')
def records_search():
   with connection:
      cursor=connection.cursor()
      token=(request.headers['Authorization'])
      user_id = check_valid_token(token, check_admin_token= True)
      if user_id==None:
         return 'invalid login'
      search_term=(request.args.get('search_term'))
      print(search_term)
      cursor.execute(f"""SELECT
                           READING.record_id,
                           BOOKS.book_name,
                           BOOKS.author,
                           USERS.email,
                           READING.date_of_start,
                           READING.date_of_end
                         FROM READING
                         JOIN BOOKS    ON READING.book_id = BOOKS.book_id
                         JOIN USERS    ON READING.user_id = USERS.user_id
                         WHERE BOOKS.book_name like "%{search_term}%" OR USERS.email like "%{search_term}%";""")
      result=cursor.fetchall()
      result_dictionary=[]
      for row in result:
        row_dictionary={'record_id':row[0],'book_name':row[1],'author':row[2],'user_email':row[3],'date_of_start':row[4], 'date_of_end':row[5]}
        result_dictionary.append(row_dictionary)

      return jsonify(result_dictionary)



if __name__ == '__main__':
   app.run(debug=True)
